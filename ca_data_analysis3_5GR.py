...
filename2 = 'cf1red.txt'

nb = 1 # The Nth block of 10 trials (1 to 6).
nf = 90 # The number of frames in a single trial.
thres_eli = 1.5 # The threshold for eliminating detected peaks that are too small. This number x standard deviation (SD) of 
...
thres_sin = 3 # The threshold for detecting peaks that are presumably caused by a single action potential.
...
frame_rate = 36 # The time (in millisecond) to capture a single frame of the movie.
us_onset = 1450 # The time (in millisecond) when the US was delivered.                                                               
...
analysis_complete = 0 # After completion of the analysis, change this value to '1' and run the code again.
...
if analysis_complete == 1:
    ...
    # Summarize the results of quantification in sheet7.
